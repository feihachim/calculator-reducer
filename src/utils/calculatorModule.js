const calculatorModule = (() => {
    ...
    function validateNumber(number) {
        ...
        const lgString = strNumber.length;
        if (lgString <= 10) {
            return strNumber;
        }
    ...
    function filterExpression(expression) {
        ...
        for (let i = 0; i < arrayCopy.length; i += 1) {
            if (OPERATOR.test(arrayCopy[i])
            && arrayCopy[i].length > 1
            && !NUMBER.test(arrayCopy[i])) {
    ...
    function operate(firstOperand, operator, secondOperand = '') {
        let result;
        const newSecondOperand = secondOperand || firstOperand;
        switch (operator) {
        case '+':
    ...
    function calculate(expression) {
        let buffer;
        const filteredEpression = filterExpression(expression);
